...
async function loadLibrary(){
    ...
    try {
        await localforage.iterate((val, key) => {
            if (!val) return;
            if (val.blob) {
                try { const src = URL.createObjectURL(val.blob); stored.push({...val, id: key, isLocal: true, src: src}); }
    ...
    let combined = [...availableDefaults, ...stored];
    ...
    combined.forEach(t => {
        ...
        // 其次：base64 缓存（作为备用，网络不可用时使用）
        const b64Cover = localStorage.getItem('am_cover_b64_' + t.id);
        if (b64Cover) { 
            t.cover = b64Cover; 
            t.coverFetched = true; 
    ...
    setTimeout(() => {
        const needsCover = combined.filter(t => {
            ...
            if (t.coverFetched) return false; // 已获取过，跳过
            if (t.cover.startsWith('data:') || t.cover.startsWith('http')) return false; // 已有有效URL，跳过
            return true;
        });
        needsCover.forEach(async t => {
            if (typeof fetchOnlineCover === 'function') {
                try {
                    ...
                    if (url) {
                        t.cover = url;
                        t.coverFetched = true;
                        localStorage.setItem('am_cover_' + t.id, url); // 兼容旧逻辑

                        // 🚀 核心：转 base64 并持久化！下次刷新直接从缓存读取
                        ...
                        // fallback：没有 base64 功能时，用原 URL 更新 UI
                        const el = document.querySelector(`.list-item[data-id="${t.id}"] .list-thumb`);
...
function saveOnlineTracks() {
    try {
        const online = playlist.filter(t => !t.isLocal);
        localStorage.setItem('am_online_tracks', JSON.stringify(online));
    } catch(e) { console.warn('saveOnlineTracks failed:', e); }
}

// Restore online tracks from localStorage on page load
function restoreOnlineTracks() {
    try {
        const stored = localStorage.getItem('am_online_tracks');
        if (!stored) return [];
        const tracks = JSON.parse(stored);
        // 确保恢复的歌曲对象有 coverFetched 属性
        tracks.forEach(t => {
...
async function importFiles(files) {
    ...
    for(let f of files) {
        ...
        newTracks.push({...meta, id: id, isLocal: true, src: URL.createObjectURL(f)});
    }
...
async function exportData() {
    const d = [];
    await localforage.iterate(v => d.push(v));
    ...
    reader.onload = async (e) => {
        try {
            ...
            if(await showConfirm(i18n[curLang].importBackup, i18n[curLang].confirmImport, i18n[curLang].importLabel, i18n[curLang].cancelLabel)) {
                ...
                localStorage.removeItem('am_hidden');
                localStorage.removeItem('am_order');
                for(const item of data) await localforage.setItem('u_' + Date.now() + '_' + Math.random(), item);
                location.reload();
            }
        } catch(err) {
...
function showSongInfo() {
    ...
    // 平台选择：默认 SONG_INFO_DEFAULT_SERVER (酷狗)，本地歌曲用 source 或默认
    ...
    const renderInfo = (track, extraHtml = '') => {
        const infoItems = [
            { label: labels.album || '专辑', value: track.album || '—' },
            { label: labels.artist || '歌手', value: track.artist || '—' },
            { label: labels.duration || '时长', value: track.duration ? fmtTime(track.duration) : '—' },
    ...
    // 判断是否需要显示搜索栏（本地歌曲 或 缺少专辑/时长/发行日期信息）
...
window.IanMusic.doSongInfoLookup = async function () {
    ...
    const renderInfo = (track, extraHtml = '') => {
        ...
        const coverHtml = track.cover
            ? `<img src="${track.cover}" style="width:100%;border-radius:12px;margin-bottom:14px;aspect-ratio:1/1;object-fit:cover;box-shadow:0 8px 32px rgba(0,0,0,0.4);" onerror="this.style.display='none'">`
            : '';
    ...
    try {
        ...
        if (msg) msg.textContent = i18n[curLang].playlistFound + (detail.album || '') + (detail.duration ? ' ' + fmtTime(detail.duration) : '');
        if (changed.album || changed.duration) {
            setTimeout(() => renderInfo(t, ''), 1200);
        }
    } catch(e) {
