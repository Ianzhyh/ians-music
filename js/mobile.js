...
function initDragHandler(element, calcCallback) {
    if (!element) return;
    let dragging = false;
    const update = e => {
        if (!dragging) return;
    ...
    element.addEventListener('pointerdown', e => {
        ...
        update(e);
        const onMove = ev => { if (!dragging) return; update(ev); };
        const onUp = () => {
            dragging = false;
...
function initMobileVol(){
    ...
    const track=wrap.querySelector('.mv-vol-track');
    initDragHandler(track, ratio => {
        audio.volume=ratio;
        document.getElementById('mv-vol-fill').style.width=(ratio*100)+'%';
        const volEl2=document.getElementById('vol');if(volEl2){volEl2.value=ratio*100;updateRangeUI(volEl2);}
    });
}
...
function _showSwipeIndicator(direction) {
    const existing = document.getElementById('mv-swipe-indicator');
    if (existing) existing.remove();
    const el = document.createElement('div');
    el.id = 'mv-swipe-indicator';
    const arrow = direction === 'left' ? '‹' : '›';
    el.textContent = arrow;
    el.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);font-size:64px;color:rgba(255,255,255,0.35);pointer-events:none;z-index:9999;font-family:var(--font-display);animation:mvSwipeFade 400ms ease-out forwards;';
    if (direction === 'left') {
        el.style.transform = 'translate(-50%,-50%) translateX(-20px)';
    } else {
        el.style.transform = 'translate(-50%,-50%) translateX(20px)';
    }
...
(function _injectSwipeCSS() {
    const s = document.createElement('style');
    s.textContent = '@keyframes mvSwipeFade{0%{opacity:1;transform:translate(-50%,-50%) translateX(0)}100%{opacity:0;transform:translate(-50%,-50%) translateX(var(--swipe-dir,0))}}';
...
function initMobileGestures() {
    ...
    mvBody.addEventListener('touchstart', e => {
        if (e.target.closest('button, a, input, [onclick]')) return;
    ...
    mvBody.addEventListener('touchmove', e => {
        if (e.target.closest('button, a, input, [onclick]')) return;
    ...
    mvBody.addEventListener('touchend', e => {
        if (e.target.closest('button, a, input, [onclick]')) return;
        ...
        const now = Date.now();
        if (!isSwiping && Math.abs(dx) < 30 && Math.abs(dy) < 30 && dt < 300) {
        ...
        if (isSwiping && Math.abs(dx) > 80 && Math.abs(dx) > Math.abs(dy) * 1.2) {
            if (dx < 0) {
                next();
                _showSwipeIndicator('left');
            } else {
                prev();
                _showSwipeIndicator('right');
            }
        }
    }, { passive: true });
}

window.IanMusic.initMobileGestures = initMobileGestures;
