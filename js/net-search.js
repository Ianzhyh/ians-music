...
async function performNetSearch() {
    ...
    window.IanMusic.addSearchHistory(query);
    ...
    try {
        ...
        if (currentSource === 'all') {
            resBox.innerHTML = SINGLE_SPINNER('全部平台');
            const result = await aggregateSearch(query);
            data = result.list || [];
        } else if (currentSource === 'bilibili') {
            resBox.innerHTML = SINGLE_SPINNER('哔哩哔哩');
            data = await bilibiliSearch(query).catch(async () => {
                console.warn('[performNetSearch] meting-api B站搜索失败，降级到公开API');
                return await bilibiliPublicSearch(query);
            });
            data = data.map(item => {
                if (!item._source) item._source = 'bilibili';
                if (!item.isBilibili && item.source === 'bilibili') item.isBilibili = true;
                return item;
            });
        ...
        if (!data || !data.length) {
            const switchBtn = currentSource === 'all' ? '' :
        ...
        if (currentSource === 'all') {
            ...
            const tags = Object.entries(sourceCounts).map(([src, cnt]) => {
                const t = PLATFORM_TAGS[src] || { label: src, color: '#888' };
                return `${t.label}${cnt}`;
            }).join(' · ');
            header.textContent = `全部平台 (${filtered.length}首) ${tags}`;
        } else {
        ...
        const showCount = Math.min(filtered.length, currentSource === 'all' ? 20 : 12);
        ...
        try {
            ...
            if (!songUrl) {
                if (server === 'bilibili' && bvid) {
                    ...
                    var _br = (typeof getQualityBitrate === 'function') ? getQualityBitrate() : 320;
                    var _brList = [_br];
                    if (_br !== 128) _brList.push(128);
                    songUrl = await window.IanMusicUtils.tryBitrate(server, itemId, _brList, metingFetch);
...
async function playMetingTrack(item, el){
    ...
    var existIdx = playlist.findIndex(function(tr) { return tr.id === newTrack.id; });
    if (existIdx !== -1) {
...
async function addItemToPlaylist(item, server, groupName, opts) {
    ...
    let songUrl = '';
    try {
        var _br2 = (typeof getQualityBitrate === 'function') ? getQualityBitrate() : 320;
        var _brList2 = [_br2];
        if (_br2 !== 128) _brList2.push(128);
        songUrl = await window.IanMusicUtils.tryBitrate(server, songId, _brList2, metingFetch) || '';
    } catch(_) {}
