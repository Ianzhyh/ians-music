...
// ================================================================
// 从 URL 取色: 优先查缓存 → 缓存命中直接用 / 缓存未命中加载原 URL
// ================================================================
...
function _init() {
    ...
    if (saved) {
        _apply(saved, _themeAuto ? document.getElementById('auto-color-btn') : null);
    } else if (_themeAuto) {
        _apply('#fa2d48', document.getElementById('auto-color-btn'));
    }
}
...
window.setAutoColorMode = async function() {
    ...
    if (!cover && trackId) {
        var b64 = window.IanMusicUtils._coverDB ? await window.IanMusicUtils._coverDB.getItem('am_cover_b64_' + trackId) : localStorage.getItem('am_cover_b64_' + trackId);
        if (b64 && b64.indexOf('data:') === 0) {
            cover = b64;
...
window.toggleCustomPicker = function() {
    ...
    if (!cp) return;
    var isActive = cp.classList.toggle('active');
    if (isActive) {
...
function _rgbToHsb(r,g,b){
    ...
    var h=0,s=mx===0?0:d/mx,br=mx;
    if(d!==0){if(mx===r)h=((g-b)/d+(g<b?6:0))*60;else if(mx===g)h=((b-r)/d+2)*60;else h=((r-g)/d+4)*60;}
    return[Math.round(h),Math.round(s*100),Math.round(br*100)];
}
function _hsbToRgb(h,s,br){s/=100;br/=100;var c=br*s,x=c*(1-Math.abs(((h/60)%2)-1)),m=br-c;var r=0,g=0,b=0;if(h<60){r=c;g=x;}else if(h<120){r=x;g=c;}else if(h<180){g=c;b=x;}else if(h<240){g=x;b=c;}else if(h<300){r=x;b=c;}else{r=c;b=x;}return[Math.round((r+m)*255),Math.round((g+m)*255),Math.round((b+m)*255)];}
...
function syncAllFromHex(hex) {
    ...
    var rEl=document.getElementById('cp-r'),gEl=document.getElementById('cp-g'),bEl=document.getElementById('cp-b');
    if(rEl){rEl.value=rgb[0];document.getElementById('cp-r-val').textContent=rgb[0];}
    if(gEl){gEl.value=rgb[1];document.getElementById('cp-g-val').textContent=rgb[1];}
    if(bEl){bEl.value=rgb[2];document.getElementById('cp-b-val').textContent=rgb[2];}
    var hEl=document.getElementById('cp-h'),sEl=document.getElementById('cp-s'),brEl=document.getElementById('cp-br');
    if(hEl){hEl.value=hsb[0];document.getElementById('cp-h-val').textContent=hsb[0];}
    if(sEl){sEl.value=hsb[1];document.getElementById('cp-s-val').textContent=hsb[1];}
    if(brEl){brEl.value=hsb[2];document.getElementById('cp-br-val').textContent=hsb[2];}
}
window.onRGBChange = function() {
    var r=parseInt(document.getElementById('cp-r').value),g=parseInt(document.getElementById('cp-g').value),b=parseInt(document.getElementById('cp-b').value);
    var hex=_rgbToHex(r,g,b);
    syncAllFromHex(hex);
    _apply(hex, document.getElementById('auto-color-btn')||null);
    addColorToHistory(hex);
};
window.onHSBChange = function() {
    var h=parseInt(document.getElementById('cp-h').value),s=parseInt(document.getElementById('cp-s').value),br=parseInt(document.getElementById('cp-br').value);
    var rgb=_hsbToRgb(h,s,br);
    var hex=_rgbToHex(rgb[0],rgb[1],rgb[2]);
    syncAllFromHex(hex);
    _apply(hex, document.getElementById('auto-color-btn')||null);
    addColorToHistory(hex);
};
window.onColorHexInput = function(hex) {
    if(!/^#[0-9a-fA-F]{6}$/.test(hex.trim())) return;
    hex=hex.trim();
    syncAllFromHex(hex);
    _apply(hex, document.getElementById('auto-color-btn')||null);
    addColorToHistory(hex);
    var el=document.getElementById('cp-preset-grid');if(el){el.querySelectorAll('.cp-preset-swatch').forEach(function(s){s.classList.remove('selected');});}
};
window.switchColorTab = function(tab) {
    ...
    if(tab==='rgb'){var v=getComputedStyle(document.documentElement).getPropertyValue('--accent').trim();syncAllFromHex(v);}
    if(tab==='hsb'){var v2=getComputedStyle(document.documentElement).getPropertyValue('--accent').trim();syncAllFromHex(v2);}
};

function _buildPresetGrid() {
    var grid=document.getElementById('cp-preset-grid');if(!grid||grid.children.length>0)return;
    var presets=['#fa2d48','#e8115b','#ff6b35','#f5a623','#ffd700','#7ed321','#50e3c2','#0a84ff','#0066cc','#5856d6','#af52de','#ff2d55','#ff9500','#34c759'];
    var cur=getComputedStyle(document.documentElement).getPropertyValue('--accent').trim();
    presets.forEach(function(p){var s=document.createElement('div');s.className='cp-preset-swatch';if(p.toLowerCase()===cur.toLowerCase())s.classList.add('selected');s.style.background=p;s.onclick=function(){onPresetClick(p,s);};grid.appendChild(s);});
}
window.onPresetClick = function(hex,el) {
    syncAllFromHex(hex);_apply(hex,document.getElementById('auto-color-btn')||null);addColorToHistory(hex);
    document.getElementById('cp-preset-grid').querySelectorAll('.cp-preset-swatch').forEach(function(s){s.classList.remove('selected');});
    if(el)el.classList.add('selected');
};

function addColorToHistory(hex) {
    var hist=[];try{hist=JSON.parse(localStorage.getItem('am_color_history')||'[]');}catch(e){}
    hist=hist.filter(function(h){return h!==hex;});hist.unshift(hex);
    if(hist.length>16)hist=hist.slice(0,16);
    try{localStorage.setItem('am_color_history',JSON.stringify(hist));}catch(e){}
    renderHistory();
}
function renderHistory(){var bar=document.getElementById('cp-history-bar');if(!bar)return;bar.innerHTML='';var hist=[];try{hist=JSON.parse(localStorage.getItem('am_color_history')||'[]');}catch(e){}hist.forEach(function(h){var d=document.createElement('div');d.className='cp-history-swatch';d.style.background=h;d.title=h;d.onclick=function(){syncAllFromHex(h);_apply(h,document.getElementById('auto-color-btn')||null);};bar.appendChild(d);});}

window.saveColorScheme = function() {
    var name=prompt('Scheme name:','My Scheme');if(!name)return;
    var hex=getComputedStyle(document.documentElement).getPropertyValue('--accent').trim();
    var schemes=[];try{schemes=JSON.parse(localStorage.getItem('am_color_schemes')||'[]');}catch(e){}
    if(schemes.length>=20){alert('Max 20 schemes');return;}
    schemes.push({name:name,hex:hex,ts:Date.now()});
    try{localStorage.setItem('am_color_schemes',JSON.stringify(schemes));}catch(e){}
    renderSchemeList();
};
window.deleteScheme = function(idx) {
    var schemes=[];try{schemes=JSON.parse(localStorage.getItem('am_color_schemes')||'[]');}catch(e){}
    if(idx>=0&&idx<schemes.length){schemes.splice(idx,1);}
    try{localStorage.setItem('am_color_schemes',JSON.stringify(schemes));}catch(e){}
    renderSchemeList();
};
function renderSchemeList(){var list=document.getElementById('cp-scheme-list');if(!list)return;list.innerHTML='';var schemes=[];try{schemes=JSON.parse(localStorage.getItem('am_color_schemes')||'[]');}catch(e){}schemes.forEach(function(s,idx){var d=document.createElement('div');d.className='cp-scheme-item';d.innerHTML='<div class="cp-scheme-dot" style="background:'+s.hex+'"></div><span class="cp-scheme-name">'+escHtml(s.name)+'</span><span class="cp-scheme-delete" onclick="event.stopPropagation();deleteScheme('+idx+')">Del</span>';d.onclick=function(){syncAllFromHex(s.hex);_apply(s.hex,document.getElementById('auto-color-btn')||null);addColorToHistory(s.hex);};list.appendChild(d);});}
