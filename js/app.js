...
async function applyNewCover(imgUrl){
    ...
    localStorage.setItem('am_cover_'+songId, finalUrl);
...
async function applyNewLyrics(lyricText,isSynced,format){
    ...
    localStorage.setItem('am_lyric_'+targetId, typeof lyricText === 'object' ? JSON.stringify(lyricText) : lyricText);
    // 🔒 将这首歌标记为手动锁定，防止后续自动搜索覆盖
...
async function init(){
    ...
    // 🌟 Restore online tracks from localStorage
    // ⚠️ 注意：loadLibrary() 内部已从 localforage 恢复本地歌曲，
    // 这里只补充恢复之前通过网络搜索添加的在线歌曲（不重复添加）
    if (typeof restoreOnlineTracks === 'function') {
        const online = restoreOnlineTracks();
        if (online.length > 0) {
            // 只添加当前 playlist 中还不存在的在线歌曲（按 id 去重）
            const existingIds = new Set(playlist.map(t => t.id));
            const newOnline = online.filter(t => !existingIds.has(t.id));
            if (newOnline.length > 0) {
                playlist.push(...newOnline);
                renderList();
                saveOrder();
            }
        }
    ...
    document.getElementById('seek')?.addEventListener('change', function() { isDrag=false; if(audio.duration&&isFinite(audio.duration))audio.currentTime=(this.value/100)*audio.duration; });
    ...
    // 初始化移动端进度/音量条交互
    window.IanMusic.initMobileSeek(); window.IanMusic.initMobileVol();
