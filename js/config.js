...
const i18n = {
    ...
    en: {
        ...
        renamePrompt: "Rename folder:", newFolderPrompt: "New folder name:", done: "Done", refreshed: "Library Refreshed",
        ...
        netSearchPlaceholder: "Song, Artist", netSearchDesc: "聚合搜索全部平台，自动过滤 Live 版，自动双 API 重试。",
    ...
    zh: {
        ...
        // ===== Existing Keys (preserved) =====
        settings: "设置", lang: "语言", accent: "主题色", size: "歌词大小", reset: "重置媒体库", library: "音乐库", file: "+ 添加文件", folder: "+ 添加文件夹", export: "导出", import: "导入", searchTitle: "更换歌词", searchBtn: "搜索", hint: "歌词将显示在这里", notfound: "未找到相关内容", move: "移动到...", create: "新建文件夹...", sleep: "睡眠定时", customSleep: "自定义 (分):", set: "设定",
        ...
        renamePrompt: "重命名文件夹:", newFolderPrompt: "新文件夹名称:", done: "完成", refreshed: "音乐库已刷新",
        ...
        netSearchPlaceholder: "歌曲名 歌手名", netSearchDesc: "聚合搜索全部平台，自动过滤 Live 版，自动双 API 重试。",
...
// ====== 内置曲目列表（从 songs.json 动态加载） ======
let myTracks = [];

function validateTracksConfig(data) {
    if (!data || typeof data !== 'object') return false;
    if (!Array.isArray(data.tracks)) return false;
    for (var i = 0; i < data.tracks.length; i++) {
        var t = data.tracks[i];
        if (!t.id || typeof t.id !== 'string') return false;
        if (!t.src || typeof t.src !== 'string') return false;
        if (!t.title || typeof t.title !== 'string') return false;
        if (!t.artist || typeof t.artist !== 'string') return false;
    }
    return true;
}
...
async function loadTracksConfig() {
    try {
        var resp = await fetch('songs.json');
        if (!resp.ok) throw new Error('HTTP ' + resp.status);
        var data = await resp.json();
        if (!validateTracksConfig(data)) {
            console.error('[loadTracksConfig] songs.json 格式验证失败');
            return;
        }
        myTracks = data.tracks;
        console.log('[loadTracksConfig] 从 songs.json 加载了', myTracks.length, '首曲目');
    } catch(e) {
        console.warn('[loadTracksConfig] 加载 songs.json 失败:', e.message, '，使用空列表');
        myTracks = [];
    }
}
...
let currentSource = localStorage.getItem('am_current_source') || 'all';
...
async function _autoRefetchAudioUrl() {
    ...
    if (t.isOffline) return;
    if (t.src && t.src.startsWith('blob:')) return;
    ...
    for (const server of platforms) {
        try {
            ...
            for (const r of results) {
                const url = await window.IanMusicUtils.tryBitrate(server, r.id, [999, 320, 128], metingFetch);
                if (url) {
                        ...
                        if (!playlist[curIdx].cover || !playlist[curIdx].coverFetched) {
                            ...
                            (window.IanMusic?.fetchOnlineCover || fetchOnlineCover)(searchTitle, searchArtist).then(hiResCover => {
                                if (hiResCover && targetIdx >= 0 && playlist[targetIdx]) {
                                    ...
                                    window.IanMusicUtils.cacheSet('am_cover_' + playlist[targetIdx].id, hiResCover);
...
window.IanMusic.loadTracksConfig = loadTracksConfig;
window.IanMusic.validateTracksConfig = validateTracksConfig;
...
function getQualityBitrate() {
    var q = 'std';
    try { q = localStorage.getItem('am_quality') || 'std'; } catch(e) {}
    switch(q) {
        case 'sq': return 999;
        case 'hq': return 320;
        case 'aac': return 256;
        case 'std': default: return 128;
    }
}
function getQualityLabel() {
    ...
    switch(q) {
        ...
        case 'hq': return 'HQ';
        case 'aac': return 'AAC';
        case 'std': default: return '标准';
    }
}
window.IanMusic.getQualityBitrate = getQualityBitrate;
window.IanMusic.getQualityLabel = getQualityLabel;
