...
async function fetchLyrics(t, a) {
    ...
    if(box) box.innerHTML = `<div style="display:flex; flex-direction:column; align-items:center; gap:15px;"><div class="spinner"></div><div id="lyric-status" class="search-result-title" style="color:var(--accent);">正在全网节点匹配歌词...</div></div>`;
    ...
    try {
        const withTimeout = (fn, ms) => {
            let timer;
            const timeout = new Promise((_, reject) => timer = setTimeout(() => reject(new Error("超时")), ms));
        ...
        // 第一梯队：本地API（QQ音乐、网易云、酷我）
        // QQ音乐QRC歌词需要搜索+获取两次请求，给10秒；网易云、酷我给5秒
        const localTimeouts = [10000, 5000, 5000];
        const localStrategyNames = ['QQ音乐', '网易云', '酷我'];
        const localStrategies = strategies.slice(0, 3);
        for (let i = 0; i < localStrategies.length; i++) {
            console.log(`[Lyrics] 尝试本地API [${i + 1}/3]: ${localStrategyNames[i]} (超时${localTimeouts[i]}ms)`);
            try {
                lyricResult = await withTimeout(localStrategies[i], localTimeouts[i]);
                if (lyricResult) {
                    console.log(`[Lyrics] ✅ 本地API成功: ${localStrategyNames[i]}`);
                    break;
                }
            } catch (err) {
                console.warn(`[Lyrics] ❌ 本地API失败 [${localStrategyNames[i]}]:`, err.message);
                lastError = err;
            }
        }

        // 第二梯队：LRCLIB（在线API，作为本地API失败后的备选）
        if (!lyricResult) {
            console.log('[Lyrics] 本地API全部失败，fallback到LRCLIB...');
            const lrclibStrategies = strategies.slice(3);
            const lrclibNames = ['LRCLIB直接获取', 'LRCLIB搜索'];
            for (let i = 0; i < lrclibStrategies.length; i++) {
                console.log(`[Lyrics] 尝试LRCLIB [${i + 1}/2]: ${lrclibNames[i]}`);
                try {
                    lyricResult = await withTimeout(lrclibStrategies[i], 8000);
                    if (lyricResult) {
                        console.log(`[Lyrics] ✅ LRCLIB成功: ${lrclibNames[i]}`);
                        break;
                    }
                } catch (err) {
                    console.warn(`[Lyrics] ❌ LRCLIB失败 [${lrclibNames[i]}]:`, err.message);
                    lastError = err;
                }
            }
        }

        if (lyricResult && myReqId === lyricReqId) {
            applyFetchedLyrics(lyricResult, targetTrackId, targetTrackIdx);
        } else {
            throw lastError || new Error("未找到歌词");
        }
    } catch (aggregateError) {
        console.log("全网彻底未找到有效滚动歌词", aggregateError?.message || '');
...
async function applyFetchedLyrics(lrcText, targetTrackId, targetTrackIdx) {
    ...
    localStorage.setItem('am_lyric_' + saveId, typeof lrcText === 'object' ? JSON.stringify(lrcText) : lrcText);
...
function renderLyrics(lrc) {
    ...
    lrc.split('\n').forEach(line=>{
        ...
        if(m&&m[3].trim()!==''){
            ...
            p.dataset.time=parseInt(m[1])*60+parseFloat(m[2].replace(':','.'));
    ...
    setTimeout(()=>{
        const deskLs=document.querySelectorAll('#desktop-lyrics-box .lyric-line');
...
function _setupLyricScroll() {
    ...
    dc.addEventListener('wheel', function(e) {
        ...
        dc.querySelectorAll('.lyric-line').forEach(function(l) { l.style.setProperty('--scroll-y', _deskScrollY + 'px'); });

        _scrollResumeTimer = setTimeout(function() {
            _userScrolling = false;
            var active = dc.querySelector('.lyric-line.highlight');
            if (active) {
...
function toggleLyricAlign(){
    ...
    document.querySelectorAll('#desktop-lyrics-box .lyric-line').forEach(l=>l.classList.toggle('left-align',_lyricLeftAlign));

    // 移动端 — 两个可能的歌词容器 + 所有歌词行
    ['#mv-lyric-rows','#mobile-lyrics-box'].forEach(sel=>{
        const mobBox=document.querySelector(sel);
        if(mobBox){
            mobBox.classList.toggle('left-align',_lyricLeftAlign);
            mobBox.querySelectorAll('.lyric-line').forEach(l=>l.classList.toggle('left-align',_lyricLeftAlign));
        }
    });
...
function showManualSearchBtn(){
    ...
    const dc=document.getElementById('desktop-lyrics-box'); if(dc) dc.innerHTML=html;
...
audio.ontimeupdate = ()=>{
    ...
    const ls=document.querySelectorAll('#desktop-lyrics-box .lyric-line');
    if(ls.length>0){
        ...
        if(activeLine&&activeLine!==_lastDesktopActive){
            ...
            if(_lastDesktopActive!==null){
                ...
                _lastDesktopActive.querySelectorAll('.lyric-char')
            ...
            if(isBackward){
                for(let i=newActiveIdx+1;i<=_lastDeskActiveIdx;i++){
                    ...
                    ls[i].querySelectorAll('.lyric-char').forEach(c=>c.style.setProperty('--char-pct','0%'));
                }
            }
            ...
            activeLine.querySelectorAll('.lyric-char').forEach(c => c.style.setProperty('--char-pct', '0%'));
            ...
            if(newActiveIdx>=0&&mobLs.length>0&&newActiveIdx!==_lastMobActiveIdx){
                ...
                if(_lastMobActiveIdx>=0&&mobLs[_lastMobActiveIdx]){
                    ...
                    mobLs[_lastMobActiveIdx].querySelectorAll('.lyric-char')
                        .forEach(c=>c.style.setProperty('--char-pct','0%'));
                }
                // 📱 新激活行：重置逐字进度，再标记 active
                mobLs[newActiveIdx].classList.add('active');
                mobLs[newActiveIdx].querySelectorAll('.lyric-char')
                    .forEach(c=>c.style.setProperty('--char-pct','0%'));
...
function _updateCharRAF(){
    if(audio.duration && audio.currentTime >= 0 && !audio.paused){
        ...
        // ---- 📱 移动端：当前 active 行逐字 ----
        const mobLs=document.querySelectorAll('#mv-lyric-rows .lyric-line.active, #mobile-lyrics-box .lyric-line.active');
        if(mobLs.length>0){
...
function _updateLineCharProgress(lineEl){
    ...
    const isDesk = lineEl.closest('#desktop-lyrics-box') !== null;
    ...
    if(audio.currentTime<lineTime) {
        lineEl.querySelectorAll('.lyric-char')
            .forEach(c => c.style.setProperty('--char-pct', '0%'));
        return;
    }
    ...
    const allLines=lineEl.closest('#desktop-lyrics-box')
        ? document.querySelectorAll('#desktop-lyrics-box .lyric-line')
        : (document.querySelectorAll('#mv-lyric-rows .lyric-line, #mobile-lyrics-box .lyric-line'));
    const idx=Array.from(allLines).indexOf(lineEl);
    if(idx>=0&&idx<allLines.length-1) nextTime=parseFloat(allLines[idx+1].dataset.time);

    const chars=lineEl.querySelectorAll('.lyric-char');
...
function renderQrcLyrics(lyrics) {
    ...
    lyrics.forEach(line => {
        ...
        for (const ch of line.chars) {
            ...
            // 每个字符独立包裹，保持正确的HTML结构
            html += `<span class="lyric-word"><span class="lyric-char" data-qrc-start="${ch.t}" data-qrc-dur="${ch.d}">${ch.c}</span></span>`;
        }
        p.innerHTML = html;
        if (line.trans) {
    ...
    if (currentTrackId != null && playlist[curIdx]) {
        ...
        localStorage.setItem('am_lyric_' + currentTrackId, json);
    }
