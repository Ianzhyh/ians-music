...
const QRLogin = {
    ...
    onStateChange(platform, oldStep, newStep) {
        console.log('[QQLogin] STATE:', oldStep, '->', newStep);
    },
    ...
    async startNeteaseLogin(onQRReady, onStatus, onSuccess, onError, onFallback) {
        ...
        try {
            onStatus('正在连接网易云...');
            const keyRes = await this._fetch('/netease/qr/key?ts=' + Date.now());
            if (!keyRes.success || !keyRes.data?.unikey) {
                const err = LOGIN_ERRORS['api_unavailable'];
                onError(err.message + ' — ' + err.suggestion);
                this._releaseLock('netease');
                return;
            }
            const key = keyRes.data.unikey;
            this._showQR(onQRReady, keyRes.data);
            onStatus('[*] 请打开"网易云音乐"APP -> 扫一扫');
            this._state['netease'].step = 'waiting';
            this._startCountdown('netease');
            this._state['netease']._onTimeout = () => {
                if (!this._state['netease']) return;
                this._checkTimeout('netease', onError, fb);
            };

            const pollFn = () => {
                this._pollTimers['netease'] = setInterval(async () => {
                    try {
                        if (this._checkTimeout('netease', onError, fb)) return;
                        const c = await this._fetch('/netease/qr/check?key=' + key + '&ts=' + Date.now());
                        if (!c.success) { console.warn('[QRLogin] Netease check !success:', c); return; }
                        const s = this._state['netease'];
                        if (!s) return;
                        s.networkErrors = 0;
                        if (c.code === 800) {
                            this._handleExpired('netease',
                                () => this.startNeteaseLogin(onQRReady, onStatus, onSuccess, onError, onFallback),
                                onStatus, onError, fb);
                            ...
                            this.stopPolling('netease');
                            const cookie = c.cookie || (c.data && c.data.cookie) || '';
                            if (cookie) {
                                this._saveCookie('netease', cookie);
                                onStatus('[OK] 登录成功');
                        ...
                        if (this._isNetworkError(e)) {
                            this._handleNetworkError('netease', pollFn, onStatus, onError);
                        }
                    }
                }, 3000);
            };
    ...
    async startKugouLogin(onQRReady, onStatus, onSuccess, onError, onFallback) {
        ...
        try {
            onStatus('正在连接酷狗...');
            const qrRes = await this._fetch('/kugou/qr/get?ts=' + Date.now());
            if (!qrRes.success || !qrRes.data?.key) {
                const err = LOGIN_ERRORS['api_unavailable'];
                onError(err.message + ' — ' + err.suggestion);
                this._releaseLock('kugou');
                return;
            }
            const key = qrRes.data.key;
            this._showQR(onQRReady, qrRes.data);
            onStatus('[*] 请使用酷狗音乐APP扫描二维码');
            this._startCountdown('kugou');
            this._state['kugou']._onTimeout = () => {
                if (!this._state['kugou']) return;
                this._checkTimeout('kugou', onError, fb);
            };

            const pollFn = () => {
                this._pollTimers['kugou'] = setInterval(async () => {
                    try {
                        if (this._checkTimeout('kugou', onError, fb)) return;
                        const c = await this._fetch('/kugou/qr/check?key=' + encodeURIComponent(key) + '&ts=' + Date.now());
                        if (!c.success) { console.warn('[QRLogin] Kugou check !success:', c); return; }
                        const st = c.data?.status;
                        console.log('[QRLogin] Kugou status:', st);
                        const s = this._state['kugou'];
                        if (!s) return;
                        s.networkErrors = 0;
                        if (st === 0) {
                            this._handleExpired('kugou',
                                () => this.startKugouLogin(onQRReady, onStatus, onSuccess, onError, onFallback),
                                onStatus, onError, fb);
                        ...
                        if (this._isNetworkError(e)) {
                            this._handleNetworkError('kugou', pollFn, onStatus, onError);
                        }
                    }
                }, 3000);
            };
