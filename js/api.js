...
const BILIBILI_API = "https://api.cenguigui.cn/api/bilibili/bilibili.php";
...
// ====== B 站独立 fetch（不走 metingFetch，避免 URL 被错误拼接）======
async function bilibiliFetch(path, timeoutMs = 8000) {
    const url = path.startsWith('http') ? path : BILIBILI_API + path;
    const controller = new AbortController();
...
async function bilibiliSearch(query) {
    const res = await bilibiliFetch(`?action=search&query=${encodeURIComponent(query)}&limit=10`);
    if (!res) return [];
    ...
    return json.data.map(item => ({
        ...
        pic: item.pic || '',
        lrc: '',
        url: item.id ? `bilibili:${item.bvid || item.id}` : '',
...
async function bilibiliGetUrl(bvid) {
    const res = await bilibiliFetch(`?action=media&bvid=${encodeURIComponent(bvid)}&quality=high`);
    if (!res) return null;
    const json = await res.json();
    if (json.code === 200 && json.data?.url) return json.data.url;
    return null;
}
...
async function aggregateSearch(query, options = {}) {
  ...
  const sources = options.sources || ['netease', 'tencent', 'kugou', 'kuwo', 'migu'];
...
window.IanMusic.bilibiliFetch = bilibiliFetch;
