...
let _fadeRAF = null;
let _userVol = prevVol;
let _audioCtx = null;
let _gainMain = null;
let _gainFade = null;
let _sourceMain = null;
let _sourceFade = null;
...
function initAudioContext() {
    ...
    _gainFade = _audioCtx.createGain();
    _sourceMain = _audioCtx.createMediaElementSource(audio);
    _sourceFade = _audioCtx.createMediaElementSource(_fadeAudio);
    _sourceMain.connect(_gainMain).connect(_audioCtx.destination);
    _sourceFade.connect(_gainFade).connect(_audioCtx.destination);
    _gainMain.gain.value = _userVol;
    _gainFade.gain.value = 0;
    audio.volume = 1.0;
    _fadeAudio.volume = 1.0;
    localStorage.setItem('am_volume', _userVol);
}

function _cancelFade() {
    if (_fadeRAF) { clearTimeout(_fadeRAF); _fadeRAF = null; }
    if (_fadeAudio) {
        _fadeAudio.pause();
        _fadeAudio.src = '';
        _fadeAudio.load();
        _fadeAudio.volume = _audioCtx ? 1.0 : 0;
    }
    if (_audioCtx) {
        var now = _audioCtx.currentTime;
        _gainMain.gain.cancelScheduledValues(now);
        _gainFade.gain.cancelScheduledValues(now);
        _gainMain.gain.setValueAtTime(_userVol, now);
        _gainFade.gain.setValueAtTime(0, now);
    }
    _fadeTargetIdx = -1;
}
...
function _startCrossfade(fromAudio, toAudio, targetIdx, duration) {
    ...
    _gainFade.gain.cancelScheduledValues(now);
    _gainMain.gain.setValueAtTime(_userVol, now);
    _gainMain.gain.linearRampToValueAtTime(0, now + duration);
    _gainFade.gain.setValueAtTime(0, now);
    _gainFade.gain.linearRampToValueAtTime(_userVol, now + duration);
    ...
    _fadeRAF = setTimeout(() => {
        ...
        var tmpSrc = _sourceMain;
        _sourceMain = _sourceFade;
        _sourceFade = tmpSrc;

        var tmpGain = _gainMain;
        _gainMain = _gainFade;
        _gainFade = tmpGain;

        _gainMain.gain.cancelScheduledValues(_audioCtx.currentTime);
        _gainMain.gain.setValueAtTime(_userVol, _audioCtx.currentTime);
        _gainFade.gain.cancelScheduledValues(_audioCtx.currentTime);
        _gainFade.gain.setValueAtTime(0, _audioCtx.currentTime);
...
function loadTrackMetaOnly(i) {
    ...
    if (coverUrl && !coverUrl.startsWith('http') && !coverUrl.startsWith('data:') && !t.coverFetched) {
        window.IanMusic.fetchSongPic(t.source || 'tencent', coverUrl, 600).then(resolvedUrl => {
            if (resolvedUrl && currentTrackId === t.id) applyCover(resolvedUrl, t.id);
        });
        coverUrl = '';
    }
    if (!coverUrl && !t.coverFetched) {
        window.IanMusic.fetchOnlineCover(t.title, t.artist).then(onlineCover => {
            if (onlineCover) { t.cover = onlineCover; t.coverFetched = true; window.IanMusicUtils.cacheSet('am_cover_' + t.id, onlineCover); if (currentTrackId === t.id) applyCover(onlineCover, t.id); }
        });
    }
        ...
        if (t.savedLyrics.startsWith('http')) {
            fetch(t.savedLyrics).then(r => r.json()).then(lrcJson => {
                ...
                if (realLrc && realLrc.includes('[00:') && currentTrackId === t.id) {
                    ...
                    window.IanMusicUtils.cacheSet('am_lyric_' + t.id, realLrc);
                    _callRender(realLrc);
                }
...
function updateVolume(el) {
    ...
    if (_audioCtx) {
        audio.volume = 1.0;
        _gainMain.gain.setValueAtTime(vol, _audioCtx.currentTime);
    } else {
        audio.volume = vol;
    }
    localStorage.setItem('am_volume', vol);
    updateRangeUI(el);
    const fill = document.getElementById('vol-fill');
...
async function loadTrack(i, autoPlay = false) {
    ...
    if (!coverUrl && !t.coverFetched) {
        ...
        window.IanMusic.fetchOnlineCover(t.title, t.artist).then(onlineCover => {
            if (onlineCover) {
                t.cover = onlineCover; 
                t.coverFetched = true;
                window.IanMusicUtils.cacheSet('am_cover_' + t.id, onlineCover);
    ...
    // 离线播放：直接从 track 对象中读取歌词（来自 IndexedDB）
    if (t.lyric !== undefined || t.tlyric !== undefined) {
        if (t.lyric && t.lyric.length > 20) {
            t.savedLyrics = t.lyric;
            if (t.id) {
                localStorage.setItem('am_lyric_' + t.id, t.lyric);
            }
        }
        if (t.tlyric) {
            if (!t.lyric && t.tlyric.length > 20) {
                t.savedLyrics = t.tlyric;
            }
        }
    }
        ...
        if(t.savedLyrics.startsWith('http')){
            try{
                ...
                if(realLrc&&realLrc.includes('[00:')){
                    if(t.isBilibili) realLrc=cleanBilibiliLyrics(realLrc);
                    playlist[curIdx].savedLyrics=realLrc; window.IanMusicUtils.cacheSet('am_lyric_'+t.id,realLrc); _callRender(realLrc);
...
function showHint(){
    const dlBox=document.getElementById('desktop-lyrics-box');if(dlBox){dlBox.innerHTML=`<div class="lyrics-hint">${i18n[curLang].hint}</div>`;dlBox.classList.add('centered-overlay');}
...
window.IanMusic.loadOfflineTrack = function(track) {
    if (track.isOffline && track.src && track.src.startsWith('blob:')) {
        track.isLocal = true;
    }
    var existIdx = playlist.findIndex(function(tr) { return tr.id === track.id; });
    if (existIdx !== -1) {
        loadTrack(existIdx, true);
        if (typeof showToast === 'function') showToast('歌曲已在播放列表中');
    } else {
        playlist.push(track);
...
if (window.appRuntime && window.appRuntime.onPlayerAction) {
    window.appRuntime.onPlayerAction(function(action) {
