...
async function getMeting() {
    if (!Meting) {
        try {
            ...
            console.error('[Meting] Failed to import @meting/core:', e.message);
            try {
                const corePath = require.resolve('@meting/core');
                const mod = await import(corePath);
                Meting = mod.default || mod;
            } catch(e2) {
                console.error('[Meting] Fallback import also failed:', e2.message);
                throw new Error('Cannot load @meting/core: ' + e.message);
            }
        }
    }
    return Meting;
...
async function handlePic(req, res, query) {
  ...
  try {
    ...
    if (proxy !== 'false') {
      try {
        let fetch;
        try { fetch = (await import('node-fetch')).default; } catch(_) { fetch = undefined; }
...
async function handleProxyImage(req, res, query) {
  ...
  try {
    let fetch;
    try { fetch = (await import('node-fetch')).default; } catch(_) { fetch = undefined; }
    const picRes = await fetch(imageUrl, { timeout: 15000 });
...
async function handleAggregateSearch(req, res, query) {
  ...
  tasks.push((async () => {
    try {
      const biliResult = await handleBilibiliSearchInternal(decodedKeyword, limit);
      return biliResult;
    } catch { return []; }
  })());

  const allPlatforms = [...metingPlatforms, 'migu', 'bilibili'];
