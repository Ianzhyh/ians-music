...
const CONFIG = {
  ...
  cookies: (function() {
    if (process.env.METING_COOKIES) {
      try { return JSON.parse(process.env.METING_COOKIES); } catch(e) {}
    }
...
async function handleSearch(req, res, query) {
  ...
  if (!platform || !keyword) {
    return errorResponse(res, 400, 'Missing required parameters: server (platform) and id (search keyword)');
  }
  
  if (!CONFIG.platformMap[platform]) {
    return errorResponse(res, 400, `Unsupported platform: ${platform}. Supported: ${Object.keys(CONFIG.platformMap).join(', ')}`);
  }
  ...
  try {
    ...
    // 支持平台 cookie（优先级：请求头 > 配置文件/环境变量）
    const platformCookie = getPlatformCookie(req, platform);
    if (platformCookie) {
      m.cookie(platformCookie);
      console.log(`[${platform.toUpperCase()}] 使用请求头 Cookie`);
    }

    // type 参数：1=歌曲(默认), 10=专辑, 100=歌手（各平台支持不同）
    const rawResult = await withTimeout(m.search(decodeURIComponent(keyword), {
      type: parseInt(type),
      page: parseInt(page),
      limit: Math.min(parseInt(limit), 50), // 上限50条
    }));
    
    console.log(`[${platform.toUpperCase()}] 搜索原始结果:`, rawResult);
    
    const data = safeJSONParse(rawResult);
    const resultCount = Array.isArray(data) ? data.length : (data?.data?.length || 0);
    
    if (resultCount === 0) {
      console.warn(`[${platform.toUpperCase()}] 搜索 "${decodeURIComponent(keyword)}" 返回空结果`);
    }
...
async function handleUrl(req, res, query) {
  ...
  if (!platform || !songId) {
    return errorResponse(res, 400, 'Missing required parameters: server and id (song ID)');
  }
  ...
  try {
    ...
    // 支持 cookie（优先级：请求头 > 配置文件/环境变量）
    const headerCookie = req.headers['x-' + platform + '-cookie'];
    const configCookie = CONFIG.cookies && CONFIG.cookies[platform];
    const platformCookie = headerCookie || configCookie || null;
    
    if (platformCookie) {
      const source = headerCookie ? '请求头' : (configCookie ? 'cookies.json' : 'env');
      const cookieLen = platformCookie.length;
      const hasUin = /uin=(\d+)/.test(platformCookie);
      const hasKey = /(?:qqmusic_key|qm_keyst)=([^;]+)/.test(platformCookie);
      console.log(`[${platform.toUpperCase()}/URL] 🍪 Cookie 已注入 (来源: ${source}, 长度: ${cookieLen}, uin: ${hasUin}, key: ${hasKey})`);
      m.cookie(platformCookie);
    } else {
      console.log(`[${platform.toUpperCase()}/URL] ⚠️ 无 Cookie — 可能无法获取 VIP/付费歌曲链接`);
      console.log(`[${platform.toUpperCase()}/URL]    提示: 将 Cookie 写入 cookies.json 的 "${platform}" 字段即可`);
    }

    // bitrate: 128 / 192 / 320 / 999 (无损 flac)
    const bitrate = parseInt(r);
    const rawResult = await withTimeout(m.url(songId, bitrate));
    const data = safeJSONParse(rawResult);
    
    // Meting 返回格式：{ url: "xxx" } 或直接字符串
    let response;
    if (typeof data === 'string') {
      response = { url: data };
    } else if (data && data.url) {
      response = data;
      // 如果返回空URL（如VIP歌曲未登录），提示需要 cookie
      if (!data.url) {
        const platformsWithCookie = Object.entries(CONFIG.cookies || {}).filter(([,v]) => v).map(([k]) => k);
        const tip = platformsWithCookie.length > 0
          ? `Empty URL — 当前平台"${platform}"${platformsWithCookie.includes(platform) ? '已配置 Cookie 但仍无法获取，可能是 Cookie 过期或该歌曲需 VIP' : '未配置 Cookie。已配置 Cookie 的平台: ' + platformsWithCookie.join(', ')}`
          : 'Empty URL — 未配置任何平台 Cookie。请将 Cookie 写入 cookies.json';
        return errorResponse(res, 403, tip, data);
      }
    } else {
...
async function handleLyric(req, res, query) {
  ...
  if (!platform || !lyricId) {
    return errorResponse(res, 400, 'Missing required parameters: server and id (lyric ID)');
  }
  ...
  try {
    ...
    // Meting 返回歌词可能是 JSON 或纯 LRC 文本
    let lyricObj;
    if (typeof rawResult === 'string' && rawResult.startsWith('{')) {
      lyricObj = safeJSONParse(rawResult);
    } else if (typeof rawResult === 'string') {
      // 纯文本歌词，尝试解析为结构化格式
      const lines = rawResult.split('\n').filter(l => l.trim());
      const hasTimeTag = /^\[\d{2}:\d{2}/.test(lines[0] || '');
      
      if (hasTimeTag) {
        // 标准 LRC 带时间标签
        lyricObj = { lrc: rawResult, type: 'lrc', lineCount: lines.length };
      } else {
        // 纯文本无时间标签
        lyricObj = { plain: rawResult, type: 'plain', lineCount: lines.length };
      }
    } else {
      lyricObj = rawResult; // 已是对象
    }
...
async function handlePic(req, res, query) {
  ...
  if (!platform || !picId) {
    return errorResponse(res, 400, 'Missing required parameters: server and id (picture ID)');
  }
  ...
  try {
    ...
    // 默认使用代理模式（解决前端 CORS 问题）
    if (proxy !== 'false') {
      try {
        const fetch = await import('node-fetch');
        const picRes = await fetch.default(finalUrl, { timeout: 10000 });
        ...
        res.writeHead(200);
        res.end(buffer);
        console.log(`[${platform.toUpperCase()}] 图片代理成功: ${finalUrl}`);
        return;
      } catch (proxyErr) {
...
async function handleProxyImage(req, res, query) {
  ...
  try {
    const fetch = await import('node-fetch');
    const picRes = await fetch.default(imageUrl, { timeout: 15000 });
...
async function handleAlbum(req, res, query) {
  ...
  if (!platform || !albumId) {
    return errorResponse(res, 400, 'Missing required parameters: server and id (album ID)');
  }
...
async function handleArtist(req, res, query) {
  ...
  if (!platform || !artistId) {
    return errorResponse(res, 400, 'Missing required parameters: server and id (artist ID)');
  }
...
async function handlePlaylist(req, res, query) {
  ...
  if (!platform || !playlistId) {
    return errorResponse(res, 400, 'Missing required parameters: server and id (playlist ID)');
  }
...
async function handleAggregateSearch(req, res, query) {
  ...
  if (!keyword) {
    return errorResponse(res, 400, 'Missing required parameter: id (search keyword)');
  }

  const decodedKeyword = decodeURIComponent(keyword);
  const metingPlatforms = Object.keys(CONFIG.platformMap); // ['netease','tencent','kugou','kuwo','baidu']
  
  console.log(`[AGGREGATE] 聚合搜索 "${decodedKeyword}" → ${metingPlatforms.join(', ')} + migu`);

  // 1. 并发搜索所有 platformMap 平台
  const tasks = metingPlatforms.map(async (platform) => {
    try {
      const m = await createMetingInstance(platform);
      const platformCookie = getPlatformCookie(req, platform);
      if (platformCookie) m.cookie(platformCookie);

      const rawResult = await withTimeout(m.search(decodedKeyword, {
        type: 1,
        page: 1,
  ...
  const allPlatforms = [...metingPlatforms, 'migu'];
...
async function handleTencentQRCheck(req, res, query) {
  ...
  try {
    ...
    if (ptCode === '0') {
      ...
      let uin = allCookies.uin || '';
      const pSkey = allCookies.p_skey || '';
      const skey = allCookies.skey || '';
      const pt4Token = allCookies.pt4_token || '';
      const qmKeyst = allCookies.qm_keyst || '';

      if (!uin) {
        const wxuin = allCookies.wxuin || '';
        if (wxuin) uin = wxuin;
      }
      if (uin) uin = uin.replace(/\D/g, '');

      const cookieParts = [];
      if (uin) cookieParts.push(`uin=${uin}`);
      if (skey) cookieParts.push(`skey=${skey}`);
      if (pSkey) cookieParts.push(`p_skey=${pSkey}`);
      if (pt4Token) cookieParts.push(`pt4_token=${pt4Token}`);
      if (pSkey) cookieParts.push(`qqmusic_key=${pSkey}`);
      if (qmKeyst) cookieParts.push(`qm_keyst=${qmKeyst}`);
