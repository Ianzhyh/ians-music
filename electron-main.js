const { app, BrowserWindow, Menu, Tray, nativeImage, dialog, ipcMain, net, session, nativeTheme, shell } = require('electron');
...
function startAPIServer() {
  return new Promise((resolve) => {
    // 优先使用 extraResources 解压路径（非 asar），确保 node_modules 可用
    let serverPath = path.join(process.resourcesPath, 'meting-api', 'server.js');
    if (!fs.existsSync(serverPath)) {
      serverPath = path.join(__dirname, 'meting-api', 'server.js');
    }
    if (!fs.existsSync(serverPath)) {
      console.error('[API] server.js not found in either resources or app directory');
      resolve(false);
      return;
    }
    ...
    const metingCoreDir = path.join(nodeModulesDir, '@meting', 'core');
    const nodeFetchDir = path.join(nodeModulesDir, 'node-fetch');

    if (!fs.existsSync(nodeModulesDir) || !fs.existsSync(metingCoreDir) || !fs.existsSync(nodeFetchDir)) {
      console.error('[API] Dependencies missing. node_modules:', fs.existsSync(nodeModulesDir), '@meting/core:', fs.existsSync(metingCoreDir), 'node-fetch:', fs.existsSync(nodeFetchDir));

      // 开发模式下自动安装依赖
      const isPackaged = !__dirname.includes('app.asar');
      if (isPackaged) {
        console.log('[API] Development mode detected, auto-installing dependencies...');
        try {
          const { execSync } = require('child_process');
          execSync('npm install', { cwd: serverDir, stdio: 'inherit', timeout: 120000 });
          console.log('[API] npm install completed');
        } catch(e) {
          console.error('[API] Auto npm install failed:', e.message);
          resolve(false);
          return;
        }
        // 重新检查
        if (!fs.existsSync(metingCoreDir) || !fs.existsSync(nodeFetchDir)) {
          console.error('[API] Dependencies still missing after npm install');
          resolve(false);
          return;
        }
      } else {
        console.error('[API] Packaged app with missing dependencies - cannot auto-install');
        resolve(false);
        return;
      }
    }

    console.log('[API] Server path:', serverPath);
    console.log('[API] Server dir:', serverDir);
    ...
    serverProcess.on('exit', (code, signal) => {
      console.error(`[API] Server exited with code ${code}, signal ${signal}`);
      if (output) {
        console.error('[API] Last output:', output.slice(-500));
      }
      if (_restartCount < MAX_RESTARTS && !isQuitting) {
        _restartCount++;
        console.log(`[API] Restarting (${_restartCount}/${MAX_RESTARTS})...`);
        setTimeout(() => {
          startAPIServer().then(resolve);
        }, 3000);
        return;
      } else if (!isQuitting) {
        console.error('[API] Max restart attempts reached');
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.webContents.send('api-error', 'API server failed to start after multiple attempts');
        }
      }
      resolve(false);
    });
    ...
    const check = (retries = 40) => {
      const req = http.get(`http://127.0.0.1:${PORT}/health`, (res) => {
        if (res.statusCode === 200) { _restartCount = 0; resolve(true); return; }
        if (retries > 0) setTimeout(() => check(retries - 1), 500);
        else resolve(false);
      });
      req.on('error', () => {
        if (retries > 0) setTimeout(() => check(retries - 1), 500);
...
function createTrayIcon() {
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    { label: 'Previous', click: () => { if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send('player-action', 'prev'); } },
    { label: 'Play / Pause', click: () => { if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send('player-action', 'toggle'); } },
    { label: 'Next', click: () => { if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send('player-action', 'next'); } },
    { type: 'separator' },
  ...
  return new Promise((resolve) => {
    ...
    const extractCookies = async () => {
      ...
      try {
        ...
        const cookieStr = _extractQQCookies(cookieMap);
        const cleanUin = (cookieMap.uin || cookieMap.wxuin || '').replace(/\D/g, '');

        if (cleanUin && cookieStr) {
...
function _extractQQCookies(raw) {
  var uin = (raw.uin || raw.wxuin || '').replace(/\D/g, '');
  var pSkey = raw.p_skey || '';
  var skey = raw.skey || '';
  var pt4Token = raw.pt4_token || '';
  var qmKeyst = raw.qm_keyst || '';
  var qqmusicKey = raw.qqmusic_key || pSkey || qmKeyst;
  var parts = [];
...
async function _qqExtractFullCookie(parts, qrsig, rawBody) {
  ...
  var cookieStr = _extractQQCookies(Object.fromEntries(fullCookieMap));
...
ipcMain.handle('qr-netease-start', async () => {
    try {
        const res = await _fetchJSON(`http://127.0.0.1:${PORT}/netease/qr/key?ts=${Date.now()}`);
...
ipcMain.handle('qr-netease-poll', async (_, key) => {
    try {
        const res = await _fetchJSON(`http://127.0.0.1:${PORT}/netease/qr/check?key=${encodeURIComponent(key)}&ts=${Date.now()}`);
        return { success: true, data: res.data };
    } catch(e) {
        return { success: false, error: e.message };
    }
...
ipcMain.handle('qr-kugou-start', async () => {
    try {
        const res = await _fetchJSON(`http://127.0.0.1:${PORT}/kugou/qr/get?ts=${Date.now()}`);
        return { success: true, data: res.data };
    } catch(e) {
        return { success: false, error: e.message };
    }
...
ipcMain.handle('qr-kugou-poll', async (_, key) => {
    try {
        const res = await _fetchJSON(`http://127.0.0.1:${PORT}/kugou/qr/check?key=${encodeURIComponent(key)}&ts=${Date.now()}`);
        return { success: true, data: res.data };
    } catch(e) {
        return { success: false, error: e.message };
    }
...
ipcMain.handle('open-path-in-explorer', async (_, filePath) => {
  try {
...
ipcMain.handle('select-folder', async () => {
  try {
    const win = BrowserWindow.getFocusedWindow() || BrowserWindow.getAllWindows().find(w => !w.isDestroyed()) || null;
...
function migrateOldSongsDir() {
  try {
    const oldDir = path.join(path.dirname(app.getPath('exe')), 'songs');
    const newDir = path.join(app.getPath('userData'), 'songs');
    ...
    if (moved > 0) {
      console.log(`[migrateOldSongsDir] 迁移了 ${moved} 个文件从 ${oldDir} 到 ${newDir}`);
      // 尝试删除旧目录（仅当为空时）
      try {
    ...
    console.warn('[migrateOldSongsDir] 迁移失败:', e.message);
  }
}

ipcMain.handle('songs:get-dir', () => {
  try { return getSongsDir(); } catch (e) { return null; }
});

ipcMain.handle('songs:save', async (_, { id, title, artist, album, cover, format, audioData }) => {
  try {
    const songsDir = getSongsDir();
    const safeName = (String(id) || Date.now().toString()).replace(/[<>:"/\\|?*]/g, '_');
    const metaPath = path.join(songsDir, safeName + '.json');
    const audioPath = path.join(songsDir, safeName + '.audio');
    ...
    console.error('[songs:save] Error:', e.message);
    return null;
  }
});
...
ipcMain.handle('songs:list', async () => {
  try {
    ...
    for (const jf of jsonFiles) {
      try {
        const raw = fs.readFileSync(path.join(songsDir, jf), 'utf-8');
        const meta = JSON.parse(raw);
        const audioFile = jf.replace('.json', '.audio');
        const audioPath = path.join(songsDir, audioFile);
...
ipcMain.handle('songs:read-audio', async (_, audioPath) => {
  try {
    if (!audioPath || !audioPath.startsWith(getSongsDir())) return null;
...
ipcMain.handle('songs:delete', async (_, id) => {
  try {
    const songsDir = getSongsDir();
    ...
    if (fs.existsSync(audioPath)) { fs.unlinkSync(audioPath); deleted = true; }
    return deleted;
  } catch (e) {
    console.error('[songs:delete] Error:', e.message);
    return false;
  }
});

ipcMain.handle('songs:update-meta', async (_, { id, lyric, tlyric }) => {
  try {
    const songsDir = getSongsDir();
    const safeName = String(id).replace(/[<>:"/\\|?*]/g, '_');
    const metaPath = path.join(songsDir, safeName + '.json');
    ...
    const meta = JSON.parse(raw);
    if (lyric !== undefined) meta.lyric = lyric;
    if (tlyric !== undefined) meta.tlyric = tlyric;
    fs.writeFileSync(metaPath, JSON.stringify(meta, null, 2), 'utf-8');
    return true;
  } catch (e) {
    return false;
...
app.whenReady().then(async () => {
  ...
  if (!serverReady) {
    ...
    dialog.showErrorBox(
      ...
      '2. 端口 3300 被占用\n' +
      '   修复: 关闭占用端口的程序\n\n' +
      '3. Node.js 版本不兼容 (需要 >= 16)\n\n' +
      '---\n' +
      'The music API server failed to start.\n' +
      'Fix: cd meting-api && npm install'
    );
    app.quit();
    return;
  }
